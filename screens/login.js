...
constructor(){
    super()
this.state={emailID:"",Password:"",IsModalVisible:false,FirstName:"",LastName:"",address:"",Contact:"",ConfirmPassword:""}
...
if(Password!==ConfirmPassword){
 ...
 firebase.auth().createUserWithEmailAndPassword(emailID,Password).then(()=>{
     db.collection("users").add({FirstName:this.state.FirstName,LastName:this.state.LastName,Contact:this.state.Contact,address:this.state.address,emailID:this.state.emailID})


        return Alert.alert("YOU ARE NOW PART OF SOMETHING GREATER",'', [ {text: 'OK', onPress: () => this.setState({"isModalVisible" : false})}, ]
)
...
<TouchableOpacity style={styles.button}onPress={()=>{this.sighn_up(this.state.emailID,this.state.Password)}}>
<Text> Sighn UP</Text>
